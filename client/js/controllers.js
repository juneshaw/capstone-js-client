...
connectbotControllers.controller('GroupShowController', ['$scope', '$routeParams', 'GroupService', function($scope, $routeParams, GroupService) {
  console.log('made it to the GroupShowController');
  $scope.groupId = $routeParams.id;
  GroupService.group($scope.groupId).first().then(function(payload) {
    console.log('the id: ', $routeParams.id);
    console.log('payload for the SHOW group', payload.data.payload);
